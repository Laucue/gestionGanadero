...
CREATE TABLE `feeding_schedules` (
  `feeding_schedule_id` int NOT NULL AUTO_INCREMENT,
  `animal_id` int NOT NULL,
  `diet_id` int NOT NULL,
  `start_date` date NOT NULL,
  `end_date` date DEFAULT NULL,
  `feeding_time` time DEFAULT NULL,
  `daily_amount_kg` decimal(8,2) NOT NULL,
  `status` enum('Activo','Finalizado','Pausado') COLLATE utf8mb4_unicode_ci DEFAULT 'Activo',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`feeding_schedule_id`),
  KEY `diet_id` (`diet_id`),
  ...
  CONSTRAINT `feeding_schedules_ibfk_2` FOREIGN KEY (`diet_id`) REFERENCES `diets` (`diet_id`) ON DELETE RESTRICT
